...
/// Editor models used by the application layer to represent the editable
/// Kahoot data in a way that maps directly to the UI (questions list on the
/// left, editor panel on the right). This layer is UI-agnostic but provides
/// convenience helpers to mutate state and to convert to/from domain entities.
...
class EditorKahoot {
  ...
  /// Add a new blank question and return its index.
  int addQuestion() {
  ...
  /// Convert editor model into domain entity Kahoot. `authorId` must be provided
  /// when creating/updating via API since the API expects `authorId` in the request.
  ...
  /// Basic validation: returns a list of error messages (empty if valid)
