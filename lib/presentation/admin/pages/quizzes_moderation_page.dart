              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      // Edit button (same icon as users)
                      IconButton(
                        icon: const Icon(Icons.edit),
                        onPressed: () async {
                          final id = k['id']?.toString();
                          if (id == null) return;
                          // Navigate to the kahoot editor page, passing the kahoot id so the
                          // editor loads existing data. Refresh list after returning.
                          await Navigator.of(context).push(MaterialPageRoute(builder: (_) => KahootEditorPage(kahootId: id)));
                          await _refresh();
                        },
                      ),
                      // Block/Activate button (use same icons as users)
                      IconButton(
                        ...
                        onPressed: () async {
                          ...
                          final isBlocking = active; // if active -> action is block
